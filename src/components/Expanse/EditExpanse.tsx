...
interface EditExpanseProps {
  opened: boolean;
  onClose: () => void;
  onUpdateExpanse: (updatedExpanse: any) => void;
  expanseData: any | null;
}
...
export const EditExpanse = ({ opened, onClose, onUpdateExpanse, expanseData }: EditExpanseProps) => {
  ...
  useEffect(() => {
    ...
    // Set the form values if expanseData is provided (i.e., when editing)
    if (expanseData) {
      console.log("expanseData.indate", expanseData.indate); // Log the date to verify its format
      form.setValues({
        Expanse_name: expanseData.Expanse_name,
        Amount: expanseData.Amount,
        Category: expanseData.Category,
        Remarks: expanseData.Remarks,
        Debit: expanseData.Debit || "0",
        Account_Title: expanseData.Account_Title,
        Account_Number: expanseData.Account_Number,
        indate: new Date(expanseData.indate).toISOString().slice(0, 10), // Ensure correct date format
      });
    }
  }, [expanseData]);
  ...
  const handleNewEntry = async (entry: string, type: "Account" | "Number") => {
    try {
      ...
      const response = await axios.post(url, { [type]: entry });
  ...
  const handleSubmit = async (values: typeof form.values) => {
    setIsLoading(true);
    try {
      await axios.put(`${API_BASE_URL}/api/expanse/update-Expanse/${expanseData._id}`, values);
      toast.success("Expanse updated successfully!");
      form.reset();
      onUpdateExpanse(values); // Pass data to parent
      onClose();
    } catch (error) {
      console.error("Error updating expanse:", error);
      toast.error("Failed to update expanse.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <>
      <Drawer opened={opened} onClose={onClose} title="Edit Expanse" padding="xl" size="sm">
        <form onSubmit={form.onSubmit(handleSubmit)}>
          ...
          <Button type="submit" mt="sm" disabled={isLoading}>
            {isLoading ? "Updating..." : "Update Expanse"}
          </Button>
        </form>
      </Drawer>
