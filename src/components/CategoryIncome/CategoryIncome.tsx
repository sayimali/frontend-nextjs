...
import DeleteCategory from "./DeleteIncome"; // Component to delete a category
import API_BASE_URL from "@/utils/config"; // Adjust the path as necessary
import { Category } from "./types"; // Import category type

// Define CategoryData type to ensure correct structure
type CategoryData = {
  _id: string;
  CategoryIncome: string;
  CategoryExpanse?: string; // Optional to match API response
};
...
export const CategoryExpanse = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_BASE_URL}/api/income/get-Category`);
      const records = response.data.records || [];
      const recordsWithRowNumbers = records.map((item: any, index: number) => ({
        ...item,
        rowNumber: index + 1,
        CategoryExpanse: item.CategoryExpanse ?? "", // Ensure it exists
      }));
  ...
  const handleAddCategory = (newCategory: CategoryData) => {
    setData((prevData) => {
      const updatedData = [
        { ...newCategory, _id: newCategory._id, rowNumber: 1, CategoryExpanse: newCategory.CategoryExpanse ?? "" },
        ...prevData,
      ];
  ...
  const handleUpdateCategory = (updatedCategory: CategoryData) => {
    setData((prevData) =>
      prevData
        .map((item) =>
          item._id === updatedCategory._id
            ? { ...updatedCategory, CategoryExpanse: updatedCategory.CategoryExpanse ?? "" }
            : item
        )
        .map((item, index) => ({
          ...item,
  ...
  const columns = useMemo<MRT_ColumnDef<CategoryData>[]>(() => [
    ...
    { accessorKey: "CategoryIncome", header: "Category Name" },
    {
      id: "actions",
      header: "Actions",
      Cell: ({ row }) => (
  ...
  return (
    <Paper withBorder radius="md" p="md">
      <Title order={5}>Income Categories</Title>
      <Space h="md" />
